...
import torch, numpy, os, re, json, shutil, types, tempfile
# import warnings
...
def dissect(outdir, model, dataset,
    ...
    with torch.no_grad():
        ...
        labeldata, ablationdata = None, None
        ...
        if make_iqr:
            ...
            assert train_dataset, 'need train dataset to do IQR thresholding'
            segloader = torch.utils.data.DataLoader(train_dataset,
                    batch_size=1, num_workers=num_workers,
                    pin_memory=(device.type == 'cuda'))
            iqrdata = collect_iqr(outdir, model, segloader,
            ...
            levels = {layer: full_iqr_levels[layer][
                    ...
                    torch.arange(max_iqr[layer].shape[1])]
        ...
        quantiles, topk = collect_quantiles_and_topk(outdir, model,
            segloader, recover_image=recover_image, k=examples_per_unit)
        ...
        if make_images:
            ...
            if levels is None:
                ...
                pass
                # Do not trust iqr-chosen level if iqr < 0.01
                # or if the unit is anticorrelated with the concept.
                # In those cases, just binarize at default quantile threshold.
                # for layer, lev in levels.items():
                #     topiqr, topiqr_index = max_iqr[layer].max(0)
                #     agreement = max_iqr_agreement[layer][topiqr_index,
                #             torch.arange(len(topiqr_index))]
                #     untrusted = (topiqr < iqr_threshold) | (agreement <= 0.5)
                #     untrusted = (agreement <= 0.5)
                #     lev[untrusted] = quantiles[layer].quantiles(
                #             [1.0 - quantile_threshold])[untrusted,0]
        ...
        if make_labels:
            segloader = torch.utils.data.DataLoader(dataset,
                    batch_size=1, num_workers=num_workers,
                    pin_memory=(device.type == 'cuda'))
            tcs, lcs, ccs, ics = collect_bincounts(model, segloader, levels,
                    recover_image=recover_image)
            iou_scores, iqr_scores = {}, {}
            for k in ics:
                iou_scores[k], iqr_scores[k] = score_tally_stats(label_category,
                        tcs, lcs, ccs[k], ics[k])
        ...
        if make_covariance:
            segloader = torch.utils.data.DataLoader(dataset,
                    ...
                    sampler=PrefixSampler(dataset, ablation_data_size),
                    num_workers=num_workers,
                    pin_memory=(device.type == 'cuda'))
            cov = collect_covariance(outdir, model, segloader,
                    recover_image=recover_image)

        if make_ablation:
            assert hasattr(model, 'ablation') and hasattr(
            ...
            segloader = torch.utils.data.DataLoader(dataset,
                    ...
                    num_workers=num_workers,
                    pin_memory=(device.type == 'cuda'))
            ablationdata = collect_ablations(outdir,
                    model, segloader, recover_image)
...
def generate_report(outdir, quantiledata, labelnames=None, catnames=None,
        ...
        ablationdata=None, netname='Model', meta=None, settings=None):
    '''
    ...
    for layer in topk.keys():
        ...
        if ablationdata is not None:
            baseline, ablated_map = ablationdata
            ablated = ablated_map[layer]
            score = (baseline).float()[None, :] - (ablated).float()
            # Label every unit by min ablation score.
            best_score, best_label = score.max(1)
            for u, urec in enumerate(units):
                urec.update(dict(
                    sua=best_score[u].item(),
                    sua_label=labelnames[best_label[u].item()][0],
                    sua_cat=labelnames[best_label[u].item()][1]
                    ))
            rankings.append(dict(name="max sua", metric="sua", score=list(
                    -ur['sua'] for ur in units)))
            # Get most popular ablation labels, and add ranking for each.
            # label_count = numpy.bincount(best_label.cpu().numpy())
            # for labelnum in numpy.argsort(-label_count):
            #     if label_count[labelnum] == 0:
            #         break
            #     label = labelnames[labelnum][0]
            #     rankings.append(dict(name="%s-sua" % label,
            #         concept=label, metric='sua',
            #         score=(-score[:, labelnum].cpu().numpy()).tolist()))
    ...
    if ablationdata is not None:
        ...
        baseline, ablated_map = ablationdata
        for label in [
                re.sub(r'-(?:t|b|l|r|tl|tr|bl|br)$', '', unitrec['sua_label'])
                for record in all_layers for unitrec in record['units']]:
            counted_labels[label] += 1
        all_labels = [label for count, label in sorted((-v, k)
            for k, v in counted_labels.items())]
...
def generate_images(outdir, model, dataset, topk, levels,
    ...
    for i, batch in enumerate(
        ...
        if hasattr(recover_image, 'recover_im_seg_bc_and_features'):
            byte_im, seg, _, torch_features, scale_offset = (
                    recover_image.recover_im_seg_bc_and_features(
...
def collect_quantiles_and_topk(outdir, model, segloader,
    ...
    cached_quantiles = {
            layer: load_quantile_if_present(os.path.join(outdir,
                safe_dir_name(layer)), 'quantiles.npz', device=device)
            for layer in model.retained }
    cached_topks = {
            layer: load_topk_if_present(os.path.join(outdir,
                safe_dir_name(layer)), 'topk.npz', device=device)
            for layer in model.retained }
    ...
    for i, batch in enumerate(progress(segloader, desc='Bincounts')):
        ...
        for key, value in features.items():
            ...
            if key not in category_activation_counts:
                ...
                category_activation_counts[key] += cc
    return (total_counts, label_counts, category_activation_counts,
            intersection_counts)

def collect_cond_quantiles(outdir, model, segloader, recover_image=None):
    '''
        ...
        for key, value in features.items():
            ...
            if key not in upsample_grids:
                upsample_grids[key] = upsample_grid(value.shape[2:],
                        ...
                        scale_offset=scale_offset_map.get(key, None)
                            if scale_offset_map is not None else None,
                        dtype=value.dtype, device=value.device)
            if key not in conditional_quantiles:
                conditional_quantiles[key] = RunningConditionalQuantile(
                        resolution=1536)
            upsampled = torch.nn.functional.grid_sample(value,
                    upsample_grids[key], padding_mode='border').view(
                            value.shape[1], -1)
            conditional_quantiles[key].add(('all',), upsampled.t())
            cpu_upsampled = None
            for label in batch_labels.tolist():
                ...
                if label_key in common_conditions:
                    ...
                    conditional_quantiles[key].add(('label', label),
                            intersected.t())
                else:
                    if cpu_seg is None:
                        cpu_seg = seg.cpu()
            ...
            if num_categories > 1:
                for cat in batch_categories.nonzero()[:,0]:
                    conditional_quantiles[key].add(('cat', cat), upsampled.t())
        # Move the most common conditions to the GPU.
        if i and not i & (i - 1):  # if i is a power of 2:
            common_conditions = set(
                conditional_quantiles[key].most_common_conditions(128))
...
def collect_maxiou(outdir, model, segloader, recover_image=None):
    '''
    Returns maxiou and maxiou_level across the data set, one per layer.
    ...
    conditional_quantiles, label_fracs = collect_cond_quantiles(
            outdir, model, segloader, recover_image=recover_image)
...
def collect_iqr(outdir, model, segloader, recover_image=None):
    '''
    ...
    for layer, cq in progress(conditional_quantiles.items(), desc='IQR'):
        ...
        isects = (1 - cq.collected_normalize(label_list, levels).to(device)
                ) * truth
        ...
        # also don't trust mi when less than label_frac is less than 1e-3.
        ...
        positive_iqr = iqr.clone()
        positive_iqr[agreement <= 0.9] = 0
        positive_iqr[(levels <= 0.0)[None, :, :].expand(positive_iqr.shape)] = 0
        # TODO: erase any for which threshold is bad
        max_iqr[layer], level_bucket = positive_iqr.max(2)
        max_iqr_level[layer] = levels.to(device)[
                torch.arange(levels.shape[0])[None,:], level_bucket]
        max_iqr_quantile[layer] = fracs.to(device)[level_bucket]
        ...
        max_iqr_iou[layer] = matching_iou
    for layer in model.retained:
...
def collect_covariance(outdir, model, segloader, recover_image=None):
    '''
...
def collect_ablations(outdir, model, segloader, recovery):
    '''
    Ablates every unit, and measures the effect on segmentation output.
    '''
    progress = default_progress()
    device = next(model.parameters()).device
    # Load from files if already computed
    baseline_bincount = load_npy_if_present(outdir, 'baseline_segcount.npy',
            device=device)
    ablated_bincounts = {
            layer: load_npy_if_present(os.path.join(outdir,
                safe_dir_name(layer)), 'ablated_segcount.npy', device=device)
            for layer in model.ablation }
    for layer in model.ablation:
        model.ablation[layer] = None
    if baseline_bincount is 0:
        data_size = 0
        for i, batch in enumerate(progress(segloader, desc='Ablation base')):
            im, seg, batch_label_counts, features, scale_offset_map = (
                    recovery.recover_im_seg_bc_and_features(
                        batch, model))
            data_size += im.shape[0]
            baseline_bincount += batch_label_counts.float().sum(0)
        baseline_bincount = baseline_bincount.float() / data_size
        os.makedirs(outdir, exist_ok=True)
        numpy.save(os.path.join(outdir, 'baseline_segcount.npy'),
                baseline_bincount)
    else:
        # Run a single batch to get the feature dimensions.
        im, seg, batch_label_counts, features, scale_offset_map = (
            recovery.recover_im_seg_bc_and_features(
                next(iter(segloader)), model))

    for layer in progress(model.ablation, desc='Ablation layers'):
        if ablated_bincounts[layer] is 0:
            model.ablation[layer] = torch.ones(features[layer].shape[1],
                    dtype=features[layer].dtype, device=device)
            ablated_bincount = torch.zeros(features[layer].shape[1],
                    len(baseline_bincount), dtype=torch.float, device=device)
            for unit in progress(range(features[layer].shape[1]), desc='units'):
                model.ablation[layer][unit] = 0
                data_size = 0
                for i, batch in enumerate(progress(segloader)):
                    im, seg, batch_label_counts, features, scale_offset_map = (
                            recovery.recover_im_seg_bc_and_features(
                                batch, model))
                    data_size += im.shape[0]
                    ablated_bincount[unit] += batch_label_counts.float().sum(0)
                model.ablation[layer][unit] = 1
            model.ablation[layer] = None
            ablated_bincounts[layer] = ablated_bincount.float() / data_size
            os.makedirs(os.path.join(outdir, safe_dir_name(layer)),
                    exist_ok=True)
            numpy.save(os.path.join(outdir, safe_dir_name(layer),
                'ablated_segcount.npy'), ablated_bincounts[layer])
    return baseline_bincount, ablated_bincounts
...
def get_module_by_path(model, module_path):
    for path, module in model.named_modules():
        if path == module_path:
            return module
    raise ValueError('{} not found in model'.format(module_path))
...
def retain_layers(model, layer_names, add_scale_offset=False):
    '''
...
def ablate_layer_output(consult, layer, name):
    '''
    Overrides the forward method of the given layer, to postmultiply output
    by an ablation vector in the consult map, if any.
    '''
    original_forward = layer.forward
    consult[name] = None
    def new_forward(self, *inputs, **kwargs):
        x = original_forward(*inputs, **kwargs)
        a = consult.get(name, None)
        if a is not None:
            x = x * a.view(*((1, -1) + (1,) * (len(x.shape) - 2)))
        return x
    layer.forward = types.MethodType(new_forward, layer)

def ablate_layers(model, layer_names):
    '''
    Creates an 'ablation' property on the model which can hold a
    vector (one dimension per channel) for every layer which, if present,
    will be postmultiplied by the output of that layer when run.
    '''
    model.ablation = {}
    seen = set()
    aka_map = {}
    ...
    for name, layer in model.named_modules():
        if name in aka_map:
            seen.add(name)
            aka = aka_map[name]
            ablate_layer_output(model.ablation, layer, aka)
    for name in aka_map:
        assert name in seen, ('Layer %s not found' % name)

def replace_layer_output(consult, layer, name):
    '''
    Overrides the forward method of the given layer, to be replaced
    by an vector in the consult map, if any.
    '''
    original_forward = layer.forward
    consult[name] = None
    def new_forward(self, *inputs, **kwargs):
        ...
        a = consult.get(name, None)
        if a is not None:
            x = torch.ones_like(x) * a.view(*((1, -1) + (1,) * (len(x.shape) - 2)))
        return x
    layer.forward = types.MethodType(new_forward, layer)

def replace_layers(model, layer_names):
    '''
    Creates an 'replacement' property on the model which can hold a
    vector (one dimension per channel) for every layer which, if present,
    will replace the output of that layer when run.
    '''
    model.replacement = {}
    seen = set()
    aka_map = {}
    ...
    for name, layer in model.named_modules():
        if name in aka_map:
            seen.add(name)
            aka = aka_map[name]
            replace_layer_output(model.replacement, layer, aka)
    for name in aka_map:
        assert name in seen, ('Layer %s not found' % name)
...
class PrefixSampler(torch.utils.data.Sampler):
    def __init__(self, data_source, size):
        self.size = min(len(data_source), size)
